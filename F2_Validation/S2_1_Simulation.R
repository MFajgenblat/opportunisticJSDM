...
#-------------------------------------------------------------------------------
# Computing the logarithm of the (non-zero) number of visits
#-------------------------------------------------------------------------------

log_visits <- (log(visitmetadata$N_visits) - mean(log(visitmetadata$N_visits)[!is.infinite(log(visitmetadata$N_visits))]))/sd(log(visitmetadata$N_visits)[!is.infinite(log(visitmetadata$N_visits))])
log_visits[is.infinite(log_visits)] <- 0
...
datalist <- list(N_species = ncol(select(any_seen, -site, -year)),
                 ...
                 Y_site = as.numeric(visitdata$site),
                 Y_year = as.numeric(visitdata$year),
                 Y_range = 1:sum(visitmetadata$N_visits),
...
model <- cmdstan_model("Models/opportunisticJSDM_alternativemodel_1.stan", cpp_options = list(stan_threads = T))
...
model <- cmdstan_model("Models/opportunisticJSDM_alternativemodel_6.stan", cpp_options = list(stan_threads = T))
stanfit <- model$sample(data = datalist, chains = 1, iter_warmup = 100, iter_sampling = 100, refresh = 1, parallel_chains = 1, threads_per_chain = 16)
fit2 <- read_cmdstan_csv(stanfit$output_files(), format = "draws_matrix")$post_warmup_draws

#-------------------------------------------------------------------------------
# Preparatory calculations for the temporal correlation metric
#-------------------------------------------------------------------------------

temporal_trend_0 <- realization %>%
  spread_draws(temporal_trend[year,species]) %>%
  group_by(year, species) %>%
  summarize(temporal_trend = mean(temporal_trend)) %>%
  pivot_wider(names_from = year, values_from = temporal_trend) %>%
  select(-species) %>%
  as.matrix()

temporal_trend_1 <- fit1 %>%
  spread_draws(temporal_trend[year,species]) %>%
  group_by(year, species) %>%
  summarize(temporal_trend = mean(temporal_trend)) %>%
  pivot_wider(names_from = year, values_from = temporal_trend) %>%
...
#-------------------------------------------------------------------------------
# Aggregating and saving the performance metric data
#-------------------------------------------------------------------------------

Performance_metrics <- rbind(data.frame(Metric = "Trend correlation", 
                                        Model = 1:2,
                                        Value = c(mean(sapply(1:N_species, function(i) cor(temporal_trend_0[i,], temporal_trend_1[i,], method = "spearman"))),
                                                  mean(sapply(1:N_species, function(i) cor(temporal_trend_0[i,], temporal_trend_6[i,], method = "spearman"))))),
                             data.frame(Metric = "Occupancy prediction", 
                                        Model = 1:2,
                                        Value = c(auc(roc(realization[,substr(colnames(realization), 1, 2) == "Z["][which(visitmetadata$N_visits == 0)], colMeans(fit1[,substr(colnames(fit1), 1, 2) == "Z["][,which(visitmetadata$N_visits == 0)]))),
                                                  auc(roc(realization[,substr(colnames(realization), 1, 2) == "Z["][which(visitmetadata$N_visits == 0)], colMeans(fit2[,substr(colnames(fit2), 1, 2) == "Z["][,which(visitmetadata$N_visits == 0)]))))),
                             data.frame(Metric = "Occupancy predictor sign", 
                                        Model = 1:2,
                                        Value = c(auc(roc(sign(colMeans(realization[,substr(colnames(realization), 1, 15) == "occupancy_beta["]))/2+0.5, apply(fit1[,substr(colnames(fit1), 1, 15) == "occupancy_beta["], 2, function(x) mean(x > 0)))),
                                                  auc(roc(sign(colMeans(realization[,substr(colnames(realization), 1, 15) == "occupancy_beta["]))/2+0.5, apply(fit2[,substr(colnames(fit2), 1, 15) == "occupancy_beta["], 2, function(x) mean(x > 0)))))))

write.table(data.frame(timestamp = format(Sys.time(), "%Y%m%d_%H%M%S"), Performance_metrics), file = paste0("F2_Validation/Performance_metrics_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".csv"), sep = ",", row.names = FALSE, col.names = TRUE)

# Optionally, combine all data frames into a single data frame
combined_data <- bind_rows(lapply(list.files(path = "Validation/Random_sampling", pattern = "^Performance_metrics_*", full.names = TRUE), read.csv))

combined_data %>%
  mutate(Model = factor(case_when(Model == 1 ~ "Naive\nJSDM",
                                  Model == 2 ~ "JSDM for\nimperfect\ndetection"),
                        levels = c("Naive\nJSDM", "JSDM for\nimperfect\ndetection")),
         Metric = factor(case_when(Metric == "Occupancy prediction" ~ "<b>Out-of-sample<br>occupancy predictions</b><br><i>ROC-AUC</i>",
                            Metric == "Occupancy predictor sign" ~ "<b>Occupancy predictor<br>sign estimation</b><br><i>ROC-AUC</i>",
                            Metric == "Trend correlation" ~ "<b>Temporal trend<br>assessment</b><br><i>Spearman's rho</i>"),
                         levels = c("<b>Out-of-sample<br>occupancy predictions</b><br><i>ROC-AUC</i>", "<b>Occupancy predictor<br>sign estimation</b><br><i>ROC-AUC</i>", "<b>Temporal trend<br>assessment</b><br><i>Spearman's rho</i>"))) %>%
  ggplot(aes(x = factor(Model), y = Value)) +
  geom_point(aes(group = timestamp), position = position_dodge(width = 0), size = 0.2) +
  geom_path(aes(group = timestamp), position = position_dodge(width = 0), linewidth = 0.1) +
  scale_x_discrete() +
...
ggsave("Model_comparison.png", width = 16, height = 8, units = "cm", dpi = 600)
