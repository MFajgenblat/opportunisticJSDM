...
model {
  ...
  target += normal_lpdf(to_vector(detection_G) | 0, 3);
  target += uniform_lpdf(detection_phylo_fraction | 0, 1);
  target += lkj_corr_cholesky_lpdf(detection_cor_covariates_L | 2.0);
  target += normal_lpdf(detection_cov_covariates_scale | 0, 3);
  target += std_normal_lpdf(to_vector(detection_beta_std));
  ...
  target += normal_lpdf(phenology_overall_sd | 0, 3);
  target += std_normal_lpdf(phenology_overall_std);
  
  for (i in 1:N_species) {
    target += normal_lpdf(phenology_sd[i] | 0, 3);
    target += std_normal_lpdf(phenology_std[i]);
  }
  ...
  for (i in 1:N_observer_dims) {
    target += std_normal_lpdf(observer_Lambda_std[i,]);
    if (i == 1) {target += gamma_lpdf(observer_Lambda_delta[i] | 2, 1);}
    if (i > 1) {target += gamma_lpdf(observer_Lambda_delta[i] | 6, 1);}
    target += std_normal_lpdf(observer_Eta[,i]);
  }
  ...
  target += reduce_sum(partial_sum, visited_units, grainsize,
    ...
    year, site, week, observer,
    occupancy_X, occupancy_beta,
    detection_X, detection_beta,
    temporal_trend,
    phenology,
